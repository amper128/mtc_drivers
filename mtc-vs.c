...
void
vs_stop_rx(struct mtc_vs_drv *vs_dev)
{
	vs_dev->vs_rx = 0;
}
...
/* dirty code */
signed int
vs_get_mctrl(struct mtc_vs_drv *vs_dev)
{
...
unsigned int
vs_type(struct mtc_vs_drv *vs_dev)
{
...
/* dirty code */
void
vs_config_port(struct mtc_vs_drv *vs_dev, char a2)
{
	if (a2 & 1) {
		vs_dev->vs_type = 86;
	}
...
/* dirty code */
unsigned int
vs_verify_port(int a1, _DWORD *a2)
{
	char v2;	     // zf@1
...
int
vs_resume(struct mtc_vs_drv *vs_dev)
{
...
int
vs_suspend(struct mtc_vs_drv *vs_dev)
{
	dev_get_drvdata(&vs_dev->dev); // WTF?
	return 0;
}
...
/* dirty code */
void
vs_set_termios(struct uart_port *p_port, int arg2)
{
...
vs_work(void)
{
	...
	} while (!(**(*(task + 3) + 4) & 0x80000) && *(v1 + 156) != *(v1 + 160) &&
		 !(*(**(v4 - 156) + 184) & 3));
}
